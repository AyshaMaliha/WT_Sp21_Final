...
//$_SESSION['message']="";
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if(!$hasError){
			$_SESSION["user"]=$name;
			$query = "insert into user values('$id','$name','$uname','$pass','$email','$num','$add','$gender','$birth')";
			execute($query);							  
		}
	}
		
		function authenticateManager($id,$uname,$pass,$email,$num,$add,$gender,$birth){
		$query = "select * from user where ID='$id'and Name='$name' and Username='$uname' and Password='$pass' and Email='$email' and Phone_Number='$num' and Address='$add' and Gender='$gender' and Date_Of_Birth='$birth' ";
		$result = get($query);
